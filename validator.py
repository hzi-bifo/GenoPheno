...
class ValidateGenML(object):
    ...
    def parse_table(self, load_path, datatype, delimiter):
        print('here')
        if datatype == 'text':
    ...
    def check_tables(self, field, value, error):
        ...
        for idx, table in enumerate(value):
            ...
            if 'table' in table:
                ...
                instances, feature_names, matrix_representation_shape = self.parse_table(table['path'], table['datatype'], table['delimiter'])
                
                if self.nodes_in_phenotype:
                    print(F"{table['name']} has {len(instances)} instances with {len(feature_names)} features that {len(set(instances).intersection(self.nodes_in_phenotype))} of them have phenotypes")
    ...
    def create_zip_file(self, project_name):
        ...
        for name, path in self.genotype_tables.items():
