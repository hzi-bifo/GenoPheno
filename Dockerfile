...
RUN mkdir src && apt -y update && apt -y --no-install-recommends install gcc poppler-utils python3-dev xvfb libxtst6 libxcomposite1 libxcursor1 libxss1 libpci3 libasound2 libgtk2.0-0 libgconf-2-4 libnss3 && conda env create -f environment.yaml && conda clean -afy && find /opt/conda/ -follow -type f -name '*.a' -delete \
